...
\begin{itemize}[leftmargin=1.4em, nosep]
  ...
  \item \textbf{精度の上限と改善（負の結果を含む）}：DAE の厳格 $\rkc$ は，候補は top-20 に揃う（Recall@20 $=0.91$）一方，それらを\textbf{ちょうど上位 $K$ 件に「集合として」並べる}ことが律速である．top-$k$ 拡大は実用 Recall@20 を $0.97$ に高めた．他方，\textbf{Stage2 の標準的強化（hard negative マイニング・listwise 損失・出力補完特徴 gOutNov による gDom 置換）は，厳格 $\rkc$ を 0.77 前後でシード分散内でしか動かさなかった}（3 シード，差 $\le 0.006$ vs 標準偏差 $\pm 0.07$）．これは候補を\textbf{独立に}スコアする現行 MLP では集合選択を表現できないためであり，次の本質的レバーは候補集合上の相互作用を陽に扱う\textbf{構造的拡張}と判明した．
  \item \textbf{今後}：(1) 修論・論文「Set-aware graph features for multi-equation physical model retrieval」の執筆，(2) \textbf{候補集合の相互作用を陽にモデル化する構造的拡張}（候補集合への注意機構／GNN，逐次的な集合構成），(3) 変数記述の意味類似度による表記揺れの学習的吸収．
\end{itemize}
