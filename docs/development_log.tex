...
\begin{itemize}[nosep]
  \item 2026-08-31（進捗報告用の日本語全体版を作成・LLM 直接生成の帰属誤りを訂正）：
    ...
    \textbf{追記（本人指摘への対応）}：「最良重みが分かったなら最初から 0.3/0.7 と書けばよく、\S10 は不要では」との指摘を受け、日本語版の旧 \S10 を \S9 に統合した（節題「誤りの過半は第 1 段で生まれる --- 作り直すと 0.743$\to$0.780」。誤り分析$\to$介入$\to$回復を 1 節で完結）。最初から新重みを書けない理由（\S4--\S8 の全数値が旧重みの系で測られている／重みの根拠が誤り分析そのもの）は本文に 1 文で明記し、\S1 に「\S4--\S8 は現行重みでの結果」の断りを追加。今後の課題の先頭に「既定重みを 0.3/0.7 に切り替えて全実験を張り直す」を置き、ギャップ分析に \textbf{A3（re-basing、要相談）}として仕様化した（再実行は \texttt{-{}-stage1-w-*} で機械的だが、選択効果の相関 $-0.404$ や誤り分解 61.8\% など旧重み依存の数値が変わり \S6.6 一部と \S6.7 の書き換えを伴うため、切り替えは先生と合意の上で）。さらに \S2.5 冒頭の「並べ直しは何本返すかを利用者が知っている前提に立つ」の 1 文が分かりにくいとの指摘を受け、①並べ直しの出力は順位であり集合にするには採る本数が要る、②評価では正解式数 $K$ を教えている、③実際の利用者は $K$ を知らない、の 3 文に展開して書き直した（当初③に付けた構造の説明と CSTR の例は本人指示で削除）。あわせて \S2.4 に逐次選択の 1 本目の選び方（固定参照での第 2 段採点の 1 位。第 1 段の 1 位とは限らない）を明記した。
