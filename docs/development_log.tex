...
%----------------------------------------------------------------------
\begin{itemize}[nosep]
  \item 2026-09-07(5)（B1: LLM 等価判定の検証——第 2 判定器 Claude Fable 5.1 との一致度）：
    LLM 直接生成の採点が判定器 claude-opus-4-8 の等価判定に依存する点（\S6.9 第 4 項）を検証するため、
    本人指定の第 2 判定器 \texttt{claude-fable-5-1} に、第 1 判定器と\textbf{同じ 147 ケース・同じ順序・同じプロンプト}
    （\texttt{evaluate\_llm\_equiv.py} から \texttt{build\_prompt()} を切り出して共有、生成式は上位 12 件）で
    608 ペア（設定 A 183・B 279・全体 146）を再判定させた（\texttt{judge\_second\_opinion.py}、adaptive thinking、
    fallbacks は使わない＝拒否時に第 1 判定器へ落ちるのを防ぐ、拒否 0・欠測 0、全呼び出し \texttt{claude-fable-5-1} で応答、
    入力 206,296・出力 50,033 トークン、約 4.6 USD、3 ラベル並列で 7.5 分）。
    一致度は \texttt{analyze\_judge\_agreement.py}（pytest 4 本、Cohen's $\kappa$ は sklearn と照合）で算出し
    \texttt{experiments/judge\_agreement\_stats.json} に保存した。
    \textbf{結果（608 ペア）}：「等価な生成式あり」ラベルの一致率 0.880・$\kappa=0.721$、
    「上位 K 件内に等価式あり」（Recall@K の判定単位）の一致率 0.910・$\kappa=0.722$、番号まで同じ 0.875。
    不一致 73 ペアのうち 71 は「Opus 4.8 は等価と認め Fable 5.1 は認めない」で\textbf{第 2 判定器のほうが厳しい}。
    第 2 判定器のもとでの LLM 直接生成は Recall@K 設定 A 0.198（第 1 判定器 0.254）・B 0.150（0.226）・全体 0.251（0.300）、
    coverage 0.338 / 0.234 / 0.463（0.419 / 0.343 / 0.567）。判定器で水準は動くが、提案手法 0.743 / 0.689 との差の結論は変わらない
    （むしろ広がる＝表 3 の第 1 判定器の値は LLM 側に寛大な値）。
    判断規則（$\kappa \ge 0.6$ だが Recall@K の差 $> 0.03$）に従い、修論に両判定器の値を併記した：
    \S5.2 に検証の段落（608 ペア・一致率・$\kappa$・不一致の向き）、\S6.1 の LLM 段落に第 2 判定器の値、
    表 3 キャプションに「第 2 判定器では 0.198 / 0.150」、\S6.9 第 4 項を「第 2 判定器で照合したが人手検証はなし」に、
    付録 A.1 に判定器 2 種の行、参考文献に Claude Opus 4.8（発表ページ）と Claude Fable 5.1（製品ページ、いずれも実在を確認）を追加し
    初出順（Sonnet 4.5 $\to$ Opus 4.8 $\to$ Fable 5.1 $\to$ E5）を維持。第 1 章 P6 の「a second model judges」は「a separate model」に修正。
    あわせて、第 1 章の本文化（W2）で VARAT が ProcessBERT より先に初出するようになっていたため参考文献の 6・7 番を入れ替え、
    全 33 件が aux の \texttt{\textbackslash citation} 順と完全一致することを機械的に再確認した。
    \texttt{latexmk -gg} exit 0・30 ページ・未定義参照なし。人手検証（本人が 20--30 ペアを判定）は未実施＝残す。
    設計書 \texttt{docs/superpowers/specs/2026-09-07-b1-second-judge-design.md}。
  \item 2026-09-07(4)（W3: 修論第 7 章 Conclusion の本文化と Abstract の書き直し）：
    第 7 章を骨子（貢献 3 点＋将来 3 方向）から 3 段落に本文化した：(1) 何をしたか（課題の定式化・2 段階＋集合特徴・学習版逐次選択・DoF-stop）、
    (2) 何が分かったか＝貢献 3 点（可解性を保証したベンチマーク／Recall@K 0.521$\to$0.743・0.466$\to$0.689、BM25・E5 の最良混合 $\le 0.602/0.555$、LLM 0.23--0.30／
    機構＝変数の重なり・話題は第 1 段の後で冗長、介入 2 回＝学習版逐次 $+0.056$（X$\ge$8）・第 1 段再設計 0.780、DoF-stop 閉包率 94\%/88\% vs oracle 87\%/78\%）、
    (3) 限界からの今後の課題 3 方向（scheduled sampling と「式がどの変数を決めるか」の新特徴／DB 外の式・異なる記法を含む実タスクでの検証／AutoPMoB パイプラインへの統合＝抽出・等価判定の下流、シミュレータの上流）。
    「第 1 段の再設計」は A1 で実施済みのため将来課題から外し介入として記載（ギャップ分析 W3 の指示）。
    \textbf{Abstract} を初稿から全面書き直し（3 段落・約 300 語: 問題と課題／手法とデータ／結果・機構・介入・自己停止）。
    閉包率の表記は第 1 章・第 7 章・Abstract で「94\% in the mixed setting and 88\% in the synthesized-only setting（oracle 87\% / 78\%）」に統一
    （第 1 章 P6 の「0.88 and 0.94」は設定 B$\to$A 順で紛らわしかったため設定名付きに修正）。数値は第 6 章と同じ一次データ（\texttt{\% source:} 付き）。
    \texttt{latexmk -gg} exit 0・30 ページ・未定義参照なし・Overfull は既存 1 件のみ。これで第 1--7 章と Abstract の全てが本文化された。
    設計書 \texttt{docs/superpowers/specs/2026-09-07-w3-conclusion-abstract-design.md}。残: B1（LLM 判定の検証）・B2（K\_REF 感度）・W4（付録転記）・W5（圧縮設定復元）・W6（D チェック）。
  \item 2026-09-07(3)（W2: 修論第 1 章 Introduction の本文化と図 1 の新規作成）：
    骨子 P1--P8 を 8 段落に本文化した（P1 物理モデルの役割と構築の負担／P2 AutoPMoB の 4 段階と要素技術／
    P3 本研究の課題と「集合として評価しなければならない理由」を CSTR の 3 式（物質収支 ODE・反応速度式 $r_A = kC_A$・アレニウスの式 $k = A\exp(-E/RT)$、未知数 $C_A, r_A, k$ の 3 つ＝自由度ゼロ）で具体化／
    P4 手法（第 1 段の混合スコア・第 2 段の 10 特徴 MLP と集合特徴 3 つの言葉での定義・学習版逐次選択・DoF-stop）／
    P5 データと層化分割／P6 結果（古典 IR $+0.22$、BM25・E5 の最良混合 $\le 0.602 / 0.555$、LLM 0.23--0.30、機構＝変数の重なり、介入 2 回、DoF-stop の閉包率）／
    P7 貢献 3 点／P8 章構成）。骨子の常套句 "play an important role" は具体（設計・制御・デジタルツイン）に置換し、
    略語は初出で展開（CSTR・ODE・MLP・LLM・DoF）、数値は全て \texttt{experiments/*.json} と照合して \texttt{\% source:} を付けた。
    \textbf{図 1}（\texttt{figures/fig\_autopmob\_overview.pdf/.png}）を \texttt{generate\_figures\_thesis.py} の \texttt{overview()} で新規作成：
    (a) AutoPMoB の 4 段階（Documents $\to$ Extraction $\to$ Equivalence judgment $\to$ \textbf{Equation-set retrieval (this study)} $\to$ Physical model）、
    (b) 本研究の入出力（Query＝説明文＋入出力変数 $\to$ 11,146 式・361 source の DB $\to$ 文献別に色分けした CSTR の 3 式を共有変数 $r_A$・$k$ の辺で結び「3 equations, 3 unknowns: DoF 0」）。
    辺は共有変数に厳密対応（加藤ルール B1/B3）、$\tau$ は持ち込まない。pytest 2 本（生成物とラベル・辺ラベルの検証）。
    \texttt{latexmk -gg} exit 0・30 ページ（28 $\to$ 30）・未定義参照なし・Overfull は既存 1 件のみ。図は章 2 ページ目の上部に配置された（圧縮設定解除後に再確認）。
    設計書 \texttt{docs/superpowers/specs/2026-09-07-w2-introduction-design.md}。残: W3（第 7 章と Abstract）。
  \item 2026-09-07(2)（A2: 外部ベースラインの補強——BM25・密ベクトル検索 E5 を修論の評価枠で測定）：
    ギャップ分析 A2 に対応し、古典 IR（TF-IDF 0.7＋変数 Jaccard 0.3、全 DB 順位付け、学習なし）の文章スコアを
    BM25（自前実装、$k_1=1.5$, $b=0.75$, 単語ユニグラム, Lucene 形式の非負 IDF）と
    E5（\texttt{intfloat/e5-base-v2}、平均プーリング＋L2 正規化、接頭辞 query:/passage:、Hugging Face から 438 MB 取得）に差し替え、
    設定 A・B $\times$ 層化分割 10 seed で Recall@K\_correct / MAP / Recall@20 を測った
    （\texttt{external\_baselines.py}・\texttt{analyze\_external\_baselines.py}・\texttt{run\_external\_baselines.sh}、pytest 7 本）。
    分割・指標は \texttt{set\_aware\_reranker.py} / \texttt{evaluate\_multi\_eq.py} の関数を流用し、
    TF-IDF・重み $(0.7, 0.3)$ が \texttt{strat\_A/B.json} の baseline の per-seed 値を完全に再現することを回帰テストで確認した。
    BM25 のスコアは非有界、E5 のコサインは狭い帯域に集中するため、クエリごとに min--max 正規化してから変数項と混合し、
    混合重みは $w_{\mathrm{t}} \in \{1, 0.9, \ldots, 0.1, 0.05, 0\}$ の 12 点を掃引した（テスト分割で最良を選ぶ＝外部側に有利な比較）。
    \textbf{結果}：文章のみでは全スコアラーが弱く（設定 A: TF-IDF 0.120 / BM25 0.150 / E5 0.146、設定 B: 0.153 / 0.164 / 0.178）、
    変数 Jaccard 単独の 0.603 / 0.541 が支配的であった。最良混合（いずれも $w_{\mathrm{t}}=0.1$）でも
    BM25 0.601 / 0.555、E5 0.602 / 0.544（説明文のみのクエリなら 0.598 / 0.547）で、
    古典 IR の最良混合 0.607 / 0.559 と 0.02 以内に並び、変数単独を上回る幅は最大 $+0.004$（A）／$+0.018$（B）にとどまる。
    提案手法 0.743 / 0.689 は最良の BM25・E5 混合を設定 A で $+0.142$ / $+0.141$、設定 B で $+0.134$ / $+0.145$ 上回り、全 10 seed で勝つ
    （Wilcoxon $p=0.00195$、$d_z=5.7$ / 6.2（A）、9.6 / 13.4（B））。
    第 1 段の候補窓としても BM25・E5 混合の被覆率は $k=50$ で最大 0.965 / 0.961 と w30-70 の 0.967 を超えず、
    密ベクトルへの第 1 段差し替えは天井を上げない（A1 との統合は不要と判断）。
    固定重み $(0.7, 0.3)$ では min--max 正規化した文章項の実効重みが生コサインより大きく、BM25 0.247 / E5 0.428（A）に落ちる。
    「同じ重み」の意味が正規化に依存するため、表では最良混合で比較した。
    \textbf{修論への反映}：\S5.2 に外部ベースライン 2 種の定義（内部 1・外部 3 に修正）、
    \S6.1 の表 \texttt{tab:overall} に設定ごと BM25・E5 の 2 行追加と段落 1 本（文章のみ／最良混合／$(0.7,0.3)$／被覆率／検定）、
    図 \texttt{fig:comparison} のキャプションを 3 手法名で明示、\S6.9 Limitations に第 7 項（既製エンコーダ・未微調整）、
    付録 A.1 に BM25・E5 の設定 3 行、参考文献に E5（Wang ら, arXiv:2212.03533、31 件目、初出順を維持）。
    \texttt{latexmk -gg} exit 0・28 ページ・未定義引用なし。第 1・7 章（骨子）には W2/W3 で反映するためのメモを残した。
    データ：\texttt{experiments/external\_baselines\_A.json / \_B.json}（60 ラベル $\times$ per-seed）、
    \texttt{external\_baselines\_coverage.json}（被覆率 $k=10$--400）、\texttt{external\_baselines\_stats.json}（対応あり検定）。
    E5 の埋め込みキャッシュ \texttt{experiments/embeddings/} は再生成可能なので非追跡（.gitignore）。
    設計書 \texttt{docs/superpowers/specs/2026-09-07-a2-external-baselines-design.md}。
  \item 2026-09-07（リポジトリ整理：未コミット作業の取り込みと .gitignore の成文化）：
    本人指示「全ての修正を commit して」に対応し 3 コミットに整理した。
    ①\texttt{chore(repo)}: これまで名前指定ステージで避けてきた除外対象
    （\texttt{.cursor/} 156 件・experiments のログ／done マーカー／スモーク退避／シード別生データ
    \texttt{xd*}・\texttt{xg*}・\texttt{xs1*}・ゼミテンプレ配下の図と LaTeX 中間物・参照用の
    先輩修論ソース \texttt{thesis/TexSource/}・\texttt{unified\_equations.bak.json}）を
    .gitignore に成文化し、作業ツリーで削除済みだった \texttt{CLAUDE.md} の削除を確定
    （作文ルール等の内容は Claude memory に移管済み）。
    ②\texttt{feat(analysis)}: 未コミットだった \texttt{judge\_llm\_equivalence.py}
    （公平等価採点・判定 Opus 4.8）・\texttt{diagnose\_comp\_coh.py}＋
    \texttt{run\_comp\_coh\_subsets.sh}（gComp/gCoh 冗長性診断）・
    \texttt{analyze\_feature\_x\_difficulty.py} のベクター PDF 出力対応・
    \texttt{experiments/ablation\_x\_difficulty.json}（難易度クロスの統合データ 7.2MB、
    図の再現に必要）を追跡。
    ③\texttt{docs(seminar)}: ゼミテンプレのビルド基盤（\texttt{jresume.cls}・\texttt{latexmkrc}・
    \texttt{main.tex}・\texttt{reference.bib}・\texttt{Fig.eps}・旧報告 tex 2 点）と完成報告 PDF
    2 点（06-14 jresume 版 7p・08-09 改訂版 4p）を追跡。テンプレ配下 \texttt{figure/} は
    従来どおり非追跡（Overleaf zip で配布・再生成可能）。
  \item 2026-08-31（W1: 修論第 2 章 Related Work の本文化と文献 4 件 $\to$ 30 件の拡充）：
    ギャップ分析（\texttt{docs/論文化ギャップ分析\_2026-08-30.md}）の W1 に対応。骨子 4 節
    （AutoPMoB 要素技術／IR と再ランク／LLM 直接生成／数式検索）を本文化し、各節末に本研究との
    相違を明示した。文献は 4 軸の並列ウェブ検証で収集し、\textbf{全 30 件を一次ページ
    （CrossRef・arXiv・ACL Anthology・出版社・NTCIR proceedings 等）で書誌確認}（作文ルール B3
    遵守・捏造ゼロ）。収穫の柱は加納研 AutoPMoB 系 6 件：CSTR Case Study (CACE {\bf 49}, 2022)・
    Prototype (CACE {\bf 53}, 2024)・ProcessBERT (IFAC-PapersOnLine {\bf 55}, 2022；
    \textbf{金上修論の引用を公刊版に差し替え})・VARAT (J. Chem. Eng. Japan {\bf 58}, 2025)・
    DAE 等価判定 (Sci. Rep. {\bf 13}, 2023)・\textbf{式結合 (Comput. Chem. Eng. {\bf 200},
    2025)＝最近接の先行研究}で、\S2.1 に「4 要件による結合＝規則固定」vs「本研究＝選択の学習＋
    閉包での自己停止」の対比を書いた。\S2.2 は MMR との\textbf{逆符号}の対比（多様性でなく可解性）、
    \S2.3 は出所（provenance）と推論時 LLM 不使用、\S2.4 は単式検索 vs 連立閉包の集合、で締めた。
    あわせて第 1 章 P1/P2（digital twin レビュー・AutoPMoB 概念論文）、第 3 章（Hangos \&
    Cameron・Salton・Manning・Deerwester・Liu）、第 4 章（Gemini 2.5, arXiv:2507.06261）、
    第 5 章（生成 Claude Sonnet 4.5 の web 引用と判定 Claude Opus 4.8 のモデル名を本文に明記、
    R-precision に Manning）へ引用を挿入。thebibliography は引用初出順に整列し、\texttt{main.aux}
    の \verb|\citation| 順と完全一致を確認。ビルド exit 0・28 ページ（25 $\to$ 28）・未定義引用なし
    （Overfull 5 件は既存の付録目次・表由来）。検証済みだが件数規範（20〜30 件）のため見送った候補
    11 件（Nougat・Tangent-CFT・MathBERT・AI Feynman・ChemCrow・NTCIR-10・Sentence-BERT・
    Nagayama et al. NLDB 2024・Zhang et al. CACE 49・Kato \& Kano CACE 52・Gemini 1.0）は
    ギャップ分析 W1 の完了記録に書誌付きで残した（即追加可能）。
  \item 2026-08-31（進捗報告用の日本語全体版を作成・LLM 直接生成の帰属誤りを訂正）：
    本人指示「進捗報告用に Methods と Results and Discussion の日本語バージョンを」に対応し、\texttt{docs/progress\_report\_2026-08-31\_methods\_results.tex/.pdf}（8 ページ・lualatex/ltjsarticle・である調）を新規作成した。構成＝\S1 位置づけ／\S2 手法（定式化・第 1 段・10 特徴量表（加藤先生確認済みの表を流用）・逐次選択・自由度ゼロ停止）／\S3 データと評価（ケース 3 種の表・新旧呼称の対応「オリジナル→文献掲載、DAE→合成」を 1 文で明記）／\S4 全体比較／\S5 内訳と PFI／\S6 逐次選択／\S7 自由度ゼロ停止／\S8 話題の近さ（\textbf{打ち消し合い＝相殺を初めて報告}）／\S9 誤りの所在（61.8\%$\to$k=200 で逆転・誤りの 2 型）／\S10 第 1 段の作り直し（A1 実験）／\S11 限界／\S12 まとめ。\S6--\S9 はゼミ報告未掲載だった結果、\S10 は新規実験。図は既存の日本語版 \texttt{fig\_pfi\_importance} と、新規生成した \texttt{docs/figures/fig\_stage1\_redesign\_ja}（\texttt{analyze\_stage1\_sweep.py -{}-report-figure} を新設。英語版と同レイアウト・箱ひげ＋被覆率曲線）。
    あわせて\textbf{重大な帰属誤りを発見・訂正}：修論 \S5.2 等の「LLM 直接生成は生成器と判定器が同一モデル（claude-opus-4-8）ゆえ LLM 有利で保守的」という記載は誤りで、\textbf{正しくは生成 claude-sonnet-4-5-20250929・判定 claude-opus-4-8 の別モデル}（commit \texttt{5fbcf65} のコミットメッセージ「生成はSonnet 4.5」で確定。predictions JSON にはモデル欄が無く、\texttt{\% source} コメントが現行のスクリプト定数を写して誤記していた）。修論 4 箇所（\S5.2 の段落・\S5.4 出典コメント・\S6.1・表 3 キャプション・\S6.9 限界）と \texttt{docs/論文の主張\_2026-08-09.md}（過大主張回避の表・想定質問 Q4）を「判定は別の上位モデル／人手検証は未実施」に書き換えた。2026-08-30 に私が表 3 キャプションを「another LLM→同一モデル」へ直した修正は誤りだった（元の記載が正しかった）ため復元に相当する。修論再ビルド 25 ページ・未解決参照なし。日本語版の LLM 記述は訂正後の事実で執筆。
    \textbf{追記（本人指摘への対応）}：「最良重みが分かったなら最初から 0.3/0.7 と書けばよく、\S10 は不要では」との指摘を受け、日本語版の旧 \S10 を \S9 に統合した（節題「誤りの過半は第 1 段で生まれる --- 作り直すと 0.743$\to$0.780」。誤り分析$\to$介入$\to$回復を 1 節で完結）。最初から新重みを書けない理由（\S4--\S8 の全数値が旧重みの系で測られている／重みの根拠が誤り分析そのもの）は本文に 1 文で明記し、\S1 に「\S4--\S8 は現行重みでの結果」の断りを追加。今後の課題の先頭に「既定重みを 0.3/0.7 に切り替えて全実験を張り直す」を置き、ギャップ分析に \textbf{A3（re-basing、要相談）}として仕様化した（再実行は \texttt{-{}-stage1-w-*} で機械的だが、選択効果の相関 $-0.404$ や誤り分解 61.8\% など旧重み依存の数値が変わり \S6.6 一部と \S6.7 の書き換えを伴うため、切り替えは先生と合意の上で）。さらに \S2.5 冒頭の「並べ直しは何本返すかを利用者が知っている前提に立つ」の 1 文が分かりにくいとの指摘を受け、①並べ直しの出力は順位であり集合にするには採る本数が要る、②評価では正解式数 $K$ を教えている、③実際の利用者は $K$ を知らない、の 3 文に展開して書き直した（当初③に付けた構造の説明と CSTR の例は本人指示で削除）。あわせて \S2.4 に逐次選択の 1 本目の選び方（固定参照での第 2 段採点の 1 位。第 1 段の 1 位とは限らない）を明記し、未定義の内輪語「静的な並べ直し」を「前節の並べ直し」に置換した。最後に本人依頼で\textbf{ゼミテンプレ（jresume）版}を作成：\texttt{docs/ゼミlatexテンプレ/progress\_report\_2026-08-31\_methods\_results.tex/.pdf}（7 ページ、platex$\to$dvipdfmx、latexmk exit 0・未解決参照なし）。図はベクター PDF（\texttt{figure/fig\_pfi\_importance.pdf} を最新版に更新、\texttt{fig\_stage1\_redesign\_ja.pdf} を新規同梱）。Overleaf 用の自己完結 zip（.tex＋jresume.cls＋latexmkrc＋figure/*.pdf、Compiler=LaTeX で上げる）を \texttt{\textasciitilde/Desktop/seminar\_overleaf\_2026-08-31.zip} に作成した。
  \item 2026-08-30(2)（A1 実験：第 1 段の変数寄り再設計で \S6.7 の予測を検証、修論 \S6.8 を新設）：
    ギャップ分析の最優先項目 A1 を実装・実行・本文化まで完了した。
    (1)\,\textbf{実装（TDD、11 テスト）}：\texttt{stage1()} に混合重み引数 \texttt{w\_text}/\texttt{w\_var}（既定 0.7/0.3＝完全後方互換）を追加し、\texttt{set\_aware\_reranker.py} に \texttt{-{}-stage1-w-text}/\texttt{-{}-stage1-w-var} を新設して\textbf{訓練時候補生成・評価時候補生成・baseline 全 DB 順位付けの 3 箇所}へ配線（重み変更が訓練の負例分布にも一貫して効く）。既定重みでの再実行が正典 \texttt{experiments/xd/reranker-10S\_\_42.json} を per-case 423 件まで完全再現することを確認してから、w70-30 参照には既存正典（\texttt{xd/}・\texttt{strat\_A.json}・\texttt{xg\_A/train}）を再利用した。
    (2)\,\textbf{被覆率スイープ（\texttt{analyze\_stage1\_weights.py}）}：$k=50$ で w70-30 0.9203 → w50-50 0.9608／\textbf{w30-70 0.9671（全 $k$ で最良）}／w00-100 0.9610。端点 2 つは既存実測（blend 0.9203・jaccard 単独 0.9610）と一致（検算）。\textbf{最良は予測した「変数のみ」でなく中間重み}＝文章項は低重みなら限界的に有効。
    (3)\,\textbf{再訓練実験（\texttt{run\_stage1\_weights.sh}, 3 重み×baseline/reranker-10S×10 seed・設定 A・$k=50$・層化）}：reranker-10S は 0.7429 → w50-50 0.7664／\textbf{w30-70 0.7799（$+0.037$, Wilcoxon $p=0.00195$, $d_z=2.4$, 全 10 seed 改善）}／w00-100 0.7762。\textbf{4 重みの被覆率順序と最終指標順序が完全一致}。最難端 X$\ge$8 で $+0.094$＝\S6.7 が特定したボトルネック（落ちるケースは被覆 0.83）で回復が最大。baseline 自体も 0.521→約 0.60（学習なしの取り分）で、再ランカーの上乗せ $+0.18$ は同重みでも維持。上限増分 $+0.047$ の約 8 割が最終指標に到達。
    (4)\,\textbf{最強構成でも検証（\texttt{run\_stage1\_weights\_k200.sh}, 学習版 greedy・$k=200$・w30-70・10 seed）}：0.7968 → \textbf{0.8098（$+0.013$, Wilcoxon $p=0.0098$, $d_z=1.1$, 9/10 seed）}、X$\ge$8 で $+0.035$（$p=0.00195$）。$k=200$ では被覆率差が $+0.015$ に縮むが、その 88\% が最終指標に変換＝再ランカーは第 1 段が入れた正解候補をほぼ使い切る。
    (5)\,\textbf{修論への反映}：\S6.8「Second Intervention: A Variable-weighted First Stage」を新設（Limitations は \S6.9 へ）。\S6.7 末尾の「次の介入実験に指定する」を「\S6.8 が実行し確認した」に差し替え、\textbf{予測→介入→回復の第 2 の弧}（第 1 の弧＝逐次選択 \S6.4）として記述。重み選択への多重比較批判に対し「3 つの変数寄り重みすべてが全 seed で有意＝結論は最良重みの選択に依存しない」を明記。図 \texttt{fig\_stage1\_redesign}（(a) 重み別被覆率曲線・(b) $k=50$ の箱ひげ、英語）を新規生成。ビルド 25 ページ・未解決参照なし。
    統計は \texttt{experiments/stage1\_weight\_coverage.json}・\texttt{stage1\_weight\_stats.json}（per-seed 付き、集計 \texttt{analyze\_stage1\_sweep.py}）、per-seed 生データは \texttt{experiments/xs1/}・\texttt{xs1\_k200/}（非追跡）。ギャップ分析の A1 を完了打ち消しにし、次の最優先を W1（Related Work＋文献収集）に更新。
  \item 2026-08-30（修論 第 6 章 Results and Discussion の本文化・図の英語化・論文化ギャップ分析）：
    本人指示「いま得られている結果を論文に掲載できるレベルで Methods と Results and Discussion の形に文書化し、発表に欠けている実験・考察を洗い出す」に対応。
    (1)\,\textbf{第 6 章の本文化}：骨子 P1--P9 を \S6.1--\S6.8 の段落形式に全面書き換え（\texttt{ResultsAndDiscussion.tex}）。執筆時に全記載値を \texttt{experiments/*.json} と再照合し、各段落に \texttt{\% source:} コメントで一次データを残した。
    (2)\,\textbf{骨子に無かった追加}：\S6.3 に向上 $+0.222$ の 3 段分解（学習のみ $0.521\to0.601$／7 特徴 $\to0.693$（うち特化度単独 $+0.090$、Wilcoxon $p=0.00195$）／集合特徴 $\to0.743$）を \texttt{experiments/xd/} から明記。\S6.6 に\textbf{分野一致（集合版）の相殺分析}を新設：向きが合う 1{,}169 ケースで置換により $-0.012$、逆向き 414 ケースで $+0.034$（Mann--Whitney $p=1.3\times10^{-8}$）、加重和 $-0.0002$ が全体 PFI $\approx 0$ を再現（\texttt{pfi\_profile\_stats.json}、ケース集約済みで疑似反復なし）。\S6.7 に $k=200$ の誤り分解（stage1 由来 61.8\%$\to$28.4\% で主因が並べ替えに逆転、\texttt{case\_outcomes\_seed42\_k200.json}）を追記。\S6.8 に教師強制の限界を追加し \S3.4 の前方参照を回収。
    (3)\,\textbf{記述の裏取りで得た per-case 検証}：DoF 停止の完全一致は oracle-K と全 7{,}026 レコードで不一致 0（平均の偶然一致でないことを確認）。サイズ誤差は設定 B で 545/545 全件が過大方向（早期停止 0）、設定 A は 4{,}594 件中 1 件のみ早期＝「ほぼ一方向」と正確に記載（当初の「全て過大」は設定 A で誤りと判明し訂正）。
    (4)\,\textbf{図の英語化と用語統一}：\texttt{fig\_topic\_diagnosis} 英語版を新規生成（\texttt{diagnose\_topic\_features.py} に \texttt{-{}-lang en} を追加。パネル (b) に全 DB AUC の菱形を重畳し「全 DB では強いが候補内で崩れる」を 1 枚で示す。JSON 再計算はバイト一致で決定性を再確認）。\texttt{fig\_pfi} は日本語版の流用だったため \texttt{generate\_figures\_thesis.py} に英語版 \texttt{pfi()} を新設。凡例の旧用語（DAE only／Original／Multi-source）を改訂用語（synthesized only／As-published／Cross-document）へ統一（\texttt{fig\_dataset}・\texttt{fig\_method\_comparison}・\texttt{fig\_greedy\_3way}・\texttt{fig\_dof\_stop}）。表 3 キャプションの誤記（判定器は「別の LLM」でなく生成器と同一モデル）を訂正。
    (5)\,\textbf{ビルド}：\texttt{latexmk -gg main.tex} exit 0、24 ページ、未解決参照・未定義引用なし。第 3--5 章は曖昧表現の走査（常套句・無根拠の有意）で問題なし。
    (6)\,\textbf{ギャップ分析}：\texttt{docs/論文化ギャップ分析\_2026-08-30.md} を新規作成。欠けている実験を優先度付きで仕様化（A1: 第 1 段の変数寄り再設計＝\S6.7 の宣言の回収、A2: BM25・密ベクトルの外部ベースライン、B1: LLM 判定の人手検証、B2: K\_REF 感度）、執筆残（第 1・2・7 章、参考文献 4 件$\to$25--40 件、付録転記、圧縮設定復元、D 課程チェック）、投稿判断事項（投稿先・データ公開範囲）を整理し、着手順を提案。
  \item 2026-08-16（加藤先生の 7 コメント対応：進捗報告の分かりにくい箇所を修正）：
    ...
    さらに「説明文を同じ雛形から作ったため文面がほぼ同じ組」の意味が伝わらないとの指摘を受け，
    実データを数えて具体的に書き直した：\texttt{generate\_multisource\_v3.py} の
    \texttt{context\_template} が示すとおり，文献横断ケースの説明文は定型文に共有変数名を
    差し込んで生成している．設定 A の文献横断 731 件を，変数名を伏せて数えると
    \textbf{定型文は 57 通りしかなく}（最多は熱交換器と制御付き反応器の各 45 件），
    正解式が別文献の別ケースでも変数名以外は説明文が同一という組ができる．
    これが最大 0.999 の正体であり，ケースの作り方に由来しベクトルの性質ではない旨を明記した．
    \textbf{ただしこの「57 通りの定型文」という書き方は不正確だった}：本人の問い
    「文献横断ケースの正解式は 2 つだけか」を機に数え直すと，731 件の説明文は\textbf{2 系統}である．
    \texttt{multisource\_v3} 500 件が定型文 24 通り（テンプレートに共有変数名を差し込む，最多 45 件），
    \texttt{multisource\_original} 33 件＋\texttt{multisource\_random\_io} 198 件＝231 件は
    手書きの説明文 33 通りを入出力変数だけ変えた派生が再利用したもの（合計 57 通り）．
    後者は「変数名を差し込む」仕組みではないので，内訳を分けて書き直した．
    なお正解式数は 731 件中 710 件（97.1\%）が 2 式，21 件が 3 式（すべて \texttt{multisource\_original} 系）である．
    最終的に本人判断で\textbf{この (ii) の項目自体を削除}し，
    「無関係な組でも類似度が高くなる」の説明を (i) 同種の装置・現象を扱う組（CSTR どうしで 0.70）だけに絞った．
    削除して差し支えないことは数値で確認済み：無関係な組 191,662 件のうち $\cos>0.5$ は 7,340 件で，
    その内訳は話題が実際に近い組が大半（説明文が同一の組は \textbf{335 件＝4.6\%} にすぎない）．
    定型文由来の組は最大値 0.999 を作るが\textbf{典型ではない}ので，(i) だけを残しても誤導にならない．
    最後に本人要望で\textbf{10 特徴量の意味と計算方法をまとめた表}を \S 1 に追加した（表 1）。
    定義は実装（\texttt{set\_aware\_reranker.py} の \texttt{compute\_features\_with\_set} と
    \texttt{compute\_set\_aware\_features}）と照合済み：基本 7 個は
    文章類似度（TF-IDF コサイン）・意味の近さ（256 次元 SVD のコサイン）・分野の一致（分野名が説明文に現れれば 1）・
    変数の一致度 $|V_q\cap V_e|/|V_q\cup V_e|$・入力変数の被覆・出力変数の被覆・式の特化度 $|V_q\cap V_e|/|V_e|$，
    集合 3 個は補完性 $|(V_q\cap V_e)\setminus V_S|/|V_q|$・一貫性 $|V_e\cap V_S|/|V_e|$・分野の一致（集合版），
    参照集合は第 1 段の上位 \texttt{K\_REF}$=5$ 件である．
    表の追加で一度 5 ページになったため，本文中の重複した定義の括弧書きを削除し，
    \S 4 方法・\S 5・今後の課題を圧縮し，図 2 を \texttt{0.88} → \texttt{0.80} 幅にして 4 ページに戻した．
    さらに本人の指摘「文章類似度の重みは 0.7 のはずでは」を受けて\textbf{重みの表記を実装に合わせた}：
    第 1 段は \texttt{two\_stage\_query\_conditioned.py} の \texttt{stage1()} で
    \texttt{argsort(-(0.7*ts + 0.3*vs))[:k]} と計算しており，重みは 0.7 と 0.3 である．
    \S 3 の「文章類似度を重み 7 で使って」は\textbf{誤り}なので「重み 0.7 を掛けて」に修正し，
    \S 1 の「7 対 3 の重みで足した」も比なのか値なのか紛れるため
    「それぞれ 0.7 と 0.3 の重みを掛けて足した」に改めた．
    最後に本人の問い「『訓練し直さないので直接測れる』とはどういう意味か」に答えて
    \S 4 の当該文を書き換えた：\textbf{訓練し直すと他の特徴量が肩代わりできる}ため
    アブレーションは「その特徴量が無くても課題が解けるか（必要性）」を，
    PFI はモデルを固定するので「今あるモデルがその特徴量をどれだけ当てにしているか（依存）」を測る，
    という対比を本文に明示した（式の特化度で PFI $+0.617$ 対 アブレーション $+0.090$ が食い違う理由）．
    アブレーション値は \texttt{experiments/xd/} で再確認（土台 A $0.601$ → ＋式の特化度 $0.691$）．
    \textbf{加藤先生に再確認いただくため注釈版 PDF を作成}：
    \texttt{docs/annotate\_revisions\_2026-08-16.py}（PyMuPDF）で修正 10 箇所をハイライトし，
    右余白に「ご指摘・要望 → 修正内容と理由」の付箋を付けた
    （\texttt{progress\_report\_2026-08-09\_topic\_revised\_annotated.pdf}）．
    その後の本人指示で\textbf{加藤先生のご指摘で直した 7 箇所だけ}に絞り，
    付箋の中身も出所や見出し記号を外して\textbf{一言の修正理由のみ}にした
    （本人の見直しによる修正＝重み 0.7・特徴量の表・「訓練し直さない」の書き換えは
    ハイライトもコメントも付けない）。最終形はハイライト 14・付箋 7．
    最後に\textbf{ゼミ資料テンプレ（jresume）版に移植}した：
    \texttt{docs/ゼミlatexテンプレ/progress\_report\_2026-08-09\_topic.tex}．
    ltjsarticle 版から本文をそのまま移し，(1) プリアンブルを
    \texttt{jresume}（\texttt{a4j,dvipdfmx,11pt}）に差し替え，
    (2) タイトルを \texttt{\textbackslash markright}＋\texttt{center}＋\texttt{flushright} の
    テンプレ書式に，(3) 図の参照を \texttt{figures/*.png} から
    \texttt{figure/*.pdf}（ベクター）に変更した．
    図 2 点は \texttt{docs/figures/} から \texttt{ゼミlatexテンプレ/figure/} へコピー済み．
    \texttt{platex} → \texttt{dvipdfmx} でエラー 0・4 ページ・内容は ltjsarticle 版と同一．
    注釈（ハイライト）は入れていない．
    さらに Overleaf へ直接アップロードできる自己完結 zip を作成した：
    \texttt{docs/seminar\_overleaf\_20260809.zip}（\texttt{figure/} の PDF 2 点＋
    \texttt{jresume.cls}＋\texttt{.tex}＋\texttt{latexmkrc} の 6 エントリ。
    2026-08-31 版 \texttt{seminar\_overleaf\_20260831.zip} と同じ構成）。
    \textbf{展開し直して単体で検証済み}：\texttt{latexmk} が exit 0・エラー 0・
    A4 4 ページで通ることを確認した。zip 自体は従来どおり git 非追跡とする。
    Overleaf 側は Upload Project 後に Compiler を \textbf{LaTeX}（pdfLaTeX 不可）にする．
