...
\begin{itemize}[nosep]
  ...
  \item \textbf{開発記録の自動追記}：Cursor のルール（\texttt{.cursor/rules/development-log.mdc}）で、問題・解決策が出た際に \texttt{development\_log.tex} へ追記するよう指示。
  \item \textbf{コミット時の自動 push}：\texttt{.git/hooks/post-commit} で \texttt{git commit} のたびに \texttt{git push} を実行し、GitHub へ自動同期。
