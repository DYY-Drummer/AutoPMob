...
% ============================================================
\begin{itemize}
  \item \textbf{P1.} Physical models are essential for realizing digital twins\cite{DigitalTwin} and play an important role in process design and operation in the process industry.
  \begin{itemize}
  ...
  \item \textbf{P2.} To free engineers from this laborious work, our group aims to develop an automated physical model builder (AutoPMoB)\cite{AutoPMoBCSTR,AutoPMoBProto}.
  \begin{itemize}
  ...
  \begin{itemize}
    ...
    \item The database holds 11,146 equations from 361 sources; the 2,838 cases include 1,000 synthesized cases whose equation sets are closed --- and hence solvable --- by construction.
  ...
  % A2 (2026-09-07): 本文化時に BM25・密ベクトル (E5) も上回ることを 1 句で足す（§6.1: 最良混合でも A 0.601/0.602, B 0.555/0.544; 提案 0.743/0.689, 全 seed）。
  \begin{itemize}
    % source: experiments/strat_A.json / strat_B.json results.*.Recall@K_correct
    \item Recall@K improves from 0.521 to 0.743 in the mixed setting and from 0.466 to 0.689 in the hardest synthesized-only setting (both +0.22, Wilcoxon signed-rank test).
  ...
  \begin{itemize}
    \item A solvable-by-construction dataset: an 11,146-equation database, synthesized cases with zero degrees of freedom, and a stratified source-disjoint split.
    \item A set-aware two-stage retrieval method that improves Recall@K by +0.22 over classical information retrieval and outperforms direct generation by an LLM.
    \item An empirical account of the mechanism --- which features cause the improvement and where it saturates --- with causal confirmation by learned greedy selection and a closure guarantee by self-terminating DoF-stop retrieval.
  \end{itemize}
