...
def load():
    ...
    for f in sorted(glob.glob(str(XG / "*.json"))):
...
def main():
    ...
    json.dump(out, open(ROOT / "experiments" / "greedy_3way_stats.json", "w", encoding="utf-8"),
    ...
    ax.set_ylabel(f"{METRIC}（DAEのみ・seed平均）")
    ...
    ax.set_xlabel("正解式数 X (DAE)"); ax.set_ylabel(METRIC)
    ax.set_title("(b) 正解式数別：飽和端で差が開くか")
    ax.grid(alpha=0.25); ax.legend(fontsize=9); ax.set_xticks(range(1, 11))
    fig.suptitle("学習版greedy 3者比較（DAEのみ・層化分割）", fontsize=13, y=1.02)
    fig.tight_layout()
    fig.savefig(ROOT / "docs" / "figures" / "fig_greedy_3way.png", dpi=150, bbox_inches="tight")
    ...
    print("\nSaved: experiments/greedy_3way_stats.json, docs/figures/fig_greedy_3way.png")
